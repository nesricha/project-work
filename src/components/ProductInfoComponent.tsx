  ...
  return (
    <div className="p-3 md:p-10">
      <p>
        Category:{" "}
        <a
          href={`/category/${prop.product.category}`}
          className="underline hover:font-medium "
        >
          {" "}
          {prop.product.category.toUpperCase()}
        </a>
      </p>
      ...
      <div className="p-10 flex flex-row flex-wrap ">
        ...
        <div className="min-w-[40vw] p-3 lg:w-6/12 mx-auto">
          <h2 className="font-bold text-3xl py-3 text-yellow-900">
            {prop.product.title}
          </h2>

          <h3 className="text-xl pb-5">{prop.product.brand}</h3>

          <span className="font-semibold text-2xl px-1.5">
            {prop.product.price} ${" "}
          </span>

          <span className="text-lg px-1.5 line-through">{` ${oldPrice} $ `}</span>

          <p className="font-bold text-xl px-1.5 py-3 text-yellow-900">
            {" "}
            {` - ${prop.product.discountPercentage} % OFF`}{" "}
          </p>
          ...
          <p className="font-bold text-xl py-2 pt-8">
            Stock:{" "}
            <span className="text-lg px-1.5 font-normal">
              {prop.product.stock}
            </span>
          </p>
          ...
          <div className="flex flex-wrap whitespace-nowrap justify-center items-center pt-8">
            <a
              href="#"
              className="m-1 text-center w-[90%] text-yellow-900 hover:text-light-1 hover:bg-yellow-900 font-semibold border border-1 rounded-lg border-yellow-900 transition duration-150 hover:scale-[1.05] p-2"
              onClick={(event) => {
